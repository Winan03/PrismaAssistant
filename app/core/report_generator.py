...
from app.llm.rag_analyzer import RAGAnalyzer, format_apa_references_list, generate_rq_explanation
...
def create_prisma_diagram(metrics, output_dir="static"):
    """Genera un flujo PRISMA detallado con cada filtro aplicado por el pipeline."""
    try:
        ...
        source_mode = str(metrics.get("article_source_mode") or "").lower()

        raw_total = as_int(filter_counts.get("raw_total"), as_int(metrics.get("total")))
        if not raw_total and source_counts:
            raw_total = sum(as_int(v) for v in source_counts.values())
        if source_mode == "csv_only" and not source_counts:
            source_counts = {"CSV/Zotero": as_int(metrics.get("csv_import_count"), raw_total)}

        concept_total = as_int(filter_counts.get("concept_total"), as_int(metrics.get("after_dedup"), raw_total))
        duplicates_count = as_int(filter_counts.get("duplicates_count"), max(0, raw_total - concept_total))
        concept_discarded = as_int(filter_counts.get("concept_discarded"))
        candidates = as_int(filter_counts.get("candidates"), max(0, concept_total - concept_discarded))
        metadata_discarded = as_int(filter_counts.get("metadata_discarded"))
        metadata_final = as_int(filter_counts.get("final_count"), max(0, candidates - metadata_discarded))
        included = as_int(metrics.get("final_included"), as_int(metrics.get("relevant"), metadata_final))
        screening_excluded = as_int(metrics.get("screening_excluded"), max(0, metadata_final - included))

        breakdown = [
            ("Fuera de rango anual", as_int(filter_counts.get("excluded_years"))),
            ("Sin acceso abierto", as_int(filter_counts.get("excluded_open_access"))),
            ("Sin calidad academica", as_int(filter_counts.get("excluded_academic_quality"))),
            ("Idioma no seleccionado", as_int(filter_counts.get("excluded_languages"))),
            ("Revista no seleccionada", as_int(filter_counts.get("excluded_manual_journals"))),
        ]
        breakdown_text = [f"{label}: n={count}" for label, count in breakdown if count > 0]
        if metadata_discarded > 0 and not breakdown_text:
            breakdown_text = ["Filtros de metadatos aplicados"]

        source_lines = [f"{name}: n={count}" for name, count in list(source_counts.items())[:5]]
        if len(source_counts) > 5:
            source_lines.append(f"Otras fuentes: n={sum(as_int(v) for v in list(source_counts.values())[5:])}")
        if not source_lines:
            source_lines = ["Fuente no declarada"]
        ...
        def draw_box(x: float, y: float, w: float, h: float, title: str,
                     body: list[str], face: str, edge: str,
                     title_color: str = "#0f172a", body_color: str = "#334155") -> None:
            ...
            body_text = "\n".join(wrapped(line, 34) for line in body if line)
            ax.text(x + w / 2, y + h / 2 - 0.012, body_text, ha="center", va="center",
                    fontsize=9, color=body_color, linespacing=1.35)

        def draw_phase(label: str, y: float, h: float) -> None:
            patch = FancyBboxPatch(
                (0.04, y), 0.10, h,
            ...
            ax.add_patch(patch)
            font_size = 8 if len(label) > 8 else 9
            ax.text(0.09, y + h / 2, label, ha="center", va="center",
                    rotation=90, fontsize=font_size, fontweight="bold", color=colors["text"])

        def arrow_down(y1: float, y2: float) -> None:
            ax.annotate("", xy=(0.36, y2), xytext=(0.36, y1),
                        arrowprops=dict(arrowstyle="->", color=colors["line"], lw=1.4))

        def arrow_right(y: float) -> None:
            ax.annotate("", xy=(0.62, y), xytext=(0.55, y),
                        arrowprops=dict(arrowstyle="->", color=colors["line"], lw=1.2))

        ax.text(0.5, 0.965, "Diagrama de Flujo PRISMA", ha="center", va="top",
                fontsize=18, fontweight="bold", color=colors["text"])
        ax.text(0.5, 0.94, "Trazabilidad de registros, filtros y estudios incluidos",
                ha="center", va="top", fontsize=10, color=colors["muted"])

        main_x, main_w, main_h = 0.18, 0.36, 0.105
        excl_x, excl_w, excl_h = 0.66, 0.28, 0.090
        rows = [
            ("IDENTIFICACION", 0.80, "Registros identificados", [f"Total inicial: n={raw_total}", *source_lines], colors["blue"]),
            ("CRIBADO", 0.64, "Registros unicos", [f"Tras deduplicacion: n={concept_total}", "Comparacion por DOI/titulo"], "#2563eb"),
            ("CRIBADO", 0.48, "Prefiltro conceptual", [f"Registros para evaluacion: n={candidates}", "Coincidencia con pregunta, titulo y resumen"], "#0284c7"),
            ("ELEGIBILIDAD", 0.32, "Filtros de elegibilidad", [f"Registros elegibles: n={metadata_final}", "Anio, idioma, acceso, calidad y revista"], "#0f766e"),
            ("INCLUIDO", 0.16, "Estudios incluidos", [f"Sintesis final: n={included}", "Revision en cascada y decision final"], colors["green"]),
        ]

        for label, y, h in [
            ("IDENTIFICACION", 0.785, 0.135),
            ("CRIBADO", 0.48, 0.265),
            ("ELEGIBILIDAD", 0.305, 0.135),
            ("INCLUIDO", 0.145, 0.135),
        ]:
            draw_phase(label, y, h)

        for index, (_, y, title, body, color) in enumerate(rows):
            draw_box(main_x, y, main_w, main_h, title, body, "#ffffff", color, color)
            if index < len(rows) - 1:
                arrow_down(y, rows[index + 1][1] + main_h)

        for y, title, body, count in [
            (0.665, "Registros descartados", [f"Duplicados: n={duplicates_count}"], duplicates_count),
            (0.505, "Registros descartados", [f"Fuera del alcance conceptual: n={concept_discarded}"], concept_discarded),
            (0.345, "Registros descartados", [f"Por filtros: n={metadata_discarded}", *breakdown_text[:4]], metadata_discarded),
            (0.185, "Articulos excluidos", [f"Screening/elegibilidad: n={screening_excluded}", "Criterios de inclusion/exclusion"], screening_excluded),
        ]:
            if count <= 0:
                continue
            draw_box(excl_x, y, excl_w, excl_h, title, body, colors["red_bg"], colors["red"],
                     colors["red"], colors["red"])
            arrow_right(y + excl_h / 2)

        ax.text(0.5, 0.045, "Nota. Flujo generado a partir de metricas reales del pipeline PRISMA Assistant.",
                ha="center", va="center", fontsize=8, color=colors["muted"], style="italic")

        path = os.path.join(output_dir, "prisma_flow.png")
        plt.savefig(path, dpi=240, bbox_inches="tight", facecolor="white", pad_inches=0.25)
        plt.close(fig)

        logging.info(f"Diagrama PRISMA detallado generado: {path}")
        return path
...
def _create_prisma_diagram_legacy(metrics, output_dir="static"):
    """
...
def _normalise_author_list(raw_authors: Any) -> list[str]:
    ...
    if isinstance(raw_authors, list):
        return [author_to_text(author) for author in raw_authors if author_to_text(author)]
    if isinstance(raw_authors, str):
        separator = ";" if ";" in raw_authors else " and "
        return [part.strip() for part in raw_authors.split(separator) if part.strip()]
    return []


def _format_author_apa(author: str) -> str:
    clean = re.sub(r"\s+", " ", str(author or "").replace(".", ". ")).strip(" ,;")
    ...
    if "," in clean:
        ...
        given_parts = re.split(r"[\s-]+", given.strip())
    else:
        parts = clean.split()
        if len(parts) == 1:
            return parts[0]
    ...
    initials = " ".join(f"{part[0].upper()}." for part in given_parts if part and part[0].isalpha())
...
def _join_apa_authors(authors: list[str]) -> str:
    ...
    for author in authors:
        apa_author = _format_author_apa(author)
        key = apa_author.lower()
        if apa_author and key not in seen:
            formatted.append(apa_author)
            seen.add(key)
...
def format_reference_apa7(article: dict) -> str:
    authors = _normalise_author_list(article.get("authors") or article.get("author"))
    auth_str = _join_apa_authors(authors)
    title = str(article.get("title") or "Sin titulo").strip()
    journal = str(article.get("journal") or "").strip()
    year = str(article.get("year") or "n.d.").strip()
    volume = str(article.get("volume") or "").strip()
    issue = str(article.get("issue") or article.get("number") or "").strip()
...
class ScientificPDF(FPDF):
    ...
    def _wrap_reference_lines(self, text: str, first_width: float, hanging_width: float) -> list[str]:
        words = text.split()
        lines: list[str] = []
        current = ""
        width = first_width
        for word in words:
            ...
            if not current or self.get_string_width(candidate) <= width:
                current = candidate
                continue
    ...
    def reference_paragraph(self, text: str, hanging_indent: float = 12.7, line_height: float = 5.0) -> None:
        """Referencia APA 7 con sangria francesa de 1.27 cm."""
        clean = self.clean_text_for_pdf(self.safe_text(text)).strip()
        if not clean:
            return

        left = self.l_margin
        full_width = self.w - self.l_margin - self.r_margin
        hanging_width = max(20, full_width - hanging_indent)
        lines = self._wrap_reference_lines(clean, full_width, hanging_width)

        for index, line in enumerate(lines):
            if self.get_y() + line_height > self.h - self.b_margin:
                self.add_page()
            self.set_x(left if index == 0 else left + hanging_indent)
            self.cell(full_width if index == 0 else hanging_width, line_height, line, 0, 1, "L")
        self.ln(2)
...
def create_pdf_report(synthesis_data, metrics, articles, question, pdf_path):
    ...
    try:
        ...
        source_mode = str(metrics.get('article_source_mode') or '').lower()
        csv_count = int(metrics.get('csv_import_count', 0) or metrics.get('total', len(articles)) or len(articles))
        search_queries = metrics.get('search_queries', {}) or {}
        t2_data = []

        if source_mode == "csv_only":
            sources_text = (
                f"El corpus inicial se obtuvo mediante carga de un CSV curado por el usuario ({csv_count} registros). "
                "En este modo no se ejecutan cadenas de busqueda automatizadas desde el sistema; por ello la Tabla 2 "
                "documenta el origen de los registros importados. Las revistas o medios de publicacion se reportan "
                "por separado a partir de la metadata disponible, sin tratarlas como bases de datos de busqueda."
            )
            t2_headers = ["Nro.", "Origen", "Registros", "Detalle"]
            t2_data.append([
                "1",
                "CSV/Zotero",
                str(csv_count),
                "Carga manual de registros; la base de datos bibliografica primaria no esta declarada por el CSV."
            ])
            pdf.body_paragraph(sources_text)
            pdf.add_table(t2_headers, t2_data, "Tabla 2: Origen de datos importados", [10, 35, 20, 105])
        else:
            if search_queries:
                all_search_sources = list(search_queries.keys())
                sources_list_str = ", ".join(all_search_sources)
            else:
                real_sources = extract_article_sources(articles, 'databases')
                sources_list_str = ", ".join(real_sources) if real_sources else "fuentes academicas configuradas"

            if source_mode == "databases_csv":
                sources_text = (
                    f"Se combinaron busquedas automatizadas en {sources_list_str} con registros adicionales importados por CSV "
                    f"({csv_count} registros CSV). Las consultas conservadas se presentan en la Tabla 2."
                )
            else:
                sources_text = (
                    f"Para la busqueda de trabajos de investigacion se consultaron las siguientes bases de datos academicas: {sources_list_str}. "
                    "Se empleo una estrategia de busqueda multi-query con cadenas automatizadas optimizadas para cada motor. "
                    "Los detalles de las consultas utilizadas se presentan en la Tabla 2."
                )
            pdf.body_paragraph(sources_text)

            t2_headers = ["Nro.", "Fuente", "Nro. Busq.", "Cadena de busqueda"]
            row_idx = 1
            spanish_words = {
                'deteccion', 'vulnerabilidades', 'seguridad', 'codigo', 'fuente',
                'revision', 'sistematica', 'cual', 'eficacia', 'frente', 'herramientas',
                'analizar', 'describir', 'identificar', 'segun', 'literatura'
            }
            ...
            for source, queries in search_queries.items():
                ...
                for q in dict.fromkeys(queries):
                    ...
                    if any(word in q_lower for word in spanish_words):
                        continue
                    if q_lower.startswith('¿') or q_lower.startswith('cuál') or q_lower.startswith('cual'):
                        continue
            ...
            if source_mode == "databases_csv" and csv_count:
                t2_data.append([str(row_idx), "CSV/Zotero", "-", f"{csv_count} registros complementarios importados por CSV."])
                row_idx += 1

            if not t2_data:
                t2_data.append(["1", sources_list_str, "-", "No se conservaron cadenas de busqueda auditables para esta ejecucion."])

            pdf.add_table(t2_headers, t2_data, "Tabla 2: Fuentes de informacion y cadenas de busqueda", [8, 30, 15, 117])
        ...
        filter_counts = metrics.get('filter_counts') or {}
        source_mode = str(metrics.get('article_source_mode') or '').lower()
        raw_total = int(filter_counts.get('raw_total', initial_search) or initial_search)
        ...
        origin_phrase = (
            f"se importaron {metrics.get('csv_import_count', raw_total)} registros desde un CSV curado por el usuario"
            if source_mode == "csv_only"
            else f"se identificaron {raw_total} registros mediante las fuentes configuradas"
        )
        selection_text = (
            f"En primer lugar, {origin_phrase}. "
            f"Tras deduplicacion y filtros de metadatos quedaron {after_filters} registros para cribado. "
            f"El prefiltro conceptual excluyo {concept_discarded} registros y los filtros de metadatos excluyeron {metadata_discarded}. "
            f"Finalmente, tras el cribado en cascada y la revision del usuario, se incluyeron {final_count} articulos en la revision. "
            "El proceso se detalla en el diagrama PRISMA (Figura 1)."
        )
        ...
        source_mode = str(metrics.get('article_source_mode') or '').lower()
        origin_desc = (
            f"un CSV curado por el usuario ({metrics.get('csv_import_count', len(articles))} registros importados)"
            if source_mode == "csv_only"
            else f"las fuentes academicas registradas ({sources_str})"
        )
        elegibilidad_intro = (
            "Los criterios de elegibilidad corresponden a los criterios declarados por el investigador en la vista de filtros. "
            f"El universo inicial provino de {origin_desc}. "
            "El sistema aplico filtros de metadatos, prefiltro conceptual y cribado en cascada. "
            f"La muestra final usada para la sintesis fue de {metrics.get('final_included', len(articles))} estudios. "
            "Las Tablas 3 y 4 transcriben esos criterios para mantener trazabilidad metodologica."
        )
        ...
        if pdf_audit:
            qa_intro += (
                f" Se verificó texto completo en {pdf_audit.get('ready', 0)} de "
                f"{pdf_audit.get('total', len(articles))} estudios incluidos; "
                f"{pdf_audit.get('uploaded', 0)} fueron incorporados mediante carga manual de PDF."
            )
            
        qa_intro = (
            "La evaluacion de calidad se reporta como una auditoria de trazabilidad del sistema, no como una calificacion subjetiva inventada. "
            "La Tabla 5 resume evidencia verificable disponible en la sesion: disponibilidad de texto completo, conteos del flujo PRISMA, "
            "criterios declarados por el investigador y origen de los registros."
        )
        pdf.body_paragraph(qa_intro)
        
        # 🔥 TABLA 5: Criterios de Evaluación de Rigor Metodológico (DINÁMICOS POR DOMINIO)
        qa_headers = ["ID", "Criterio de evaluacion / auditoria", "Evidencia verificable en sesion"]
        qa_data = generate_dynamic_qa_criteria(question, articles, metrics)
        
        pdf.add_table(qa_headers, qa_data, "Tabla 5: Auditoria de calidad y trazabilidad", [15, 95, 60])
        ...
        try:
            ...
            pdf.body_paragraph(
                "En las tablas de aportes por pregunta de investigacion, cada articulo se asigna al aporte predominante "
                "que mejor responde a la pregunta correspondiente. Por ello, el total de cada tabla coincide con los articulos clasificados para esa PI."
            )
            ...
            for rq_row in rqs_data:
                ...
                rq_question_clean = re.sub(rf"^\s*PI{rq_num}\s*:\s*", "", str(rq_question), flags=re.IGNORECASE)
                ...
                pdf.ln(4)
                pdf.set_font('Arial', 'B', 10)
                pdf.multi_cell(0, 5, f"PI{rq_num}: {rq_question_clean}")
                pdf.ln(2)
                ...
                if categories:
                    ...
                    rq_table_headers = ["Nro.", "Aportes", "Cantidad", "Referencias", "Porcentaje (%)"]
                    rq_table_data = []
                    displayed_total = sum(int(cat.get('count', 0) or 0) for cat in categories)
                    
                    for i, cat in enumerate(categories, 1):
                        cat_name = cat.get('name', 'Sin clasificar')
                        cat_count = int(cat.get('count', 0) or 0)
                        cat_pct = (cat_count / displayed_total * 100) if displayed_total > 0 else 0.0
                    ...
                    caption = f"Tabla {table_num}: Aportes de {rq_theme} para PI{rq_num}"
                    pdf.add_table(rq_table_headers, rq_table_data, caption, [10, 50, 20, 75, 15])
                    table_num += 1
...
A pesar de los avances logrados en la aplicación de {topic}, persisten desafíos que requieren atención, siendo esencial priorizar las necesidades del usuario en el desarrollo de soluciones más efectivas. Se identificó una notable escasez de estudios longitudinales y enfoques comparativos entre diferentes contextos, lo que representa una oportunidad significativa para futuras investigaciones. La combinación de diferentes enfoques metodológicos podría mejorar la comprensión del fenómeno; se sugiere que los estudios futuros exploren metodologías innovadoras que optimicen el diseño de soluciones y redefinan las prácticas actuales en un entorno tecnológico en constante evolución, permitiendo así abordar mejor los problemas actuales y avanzar hacia implementaciones más efectivas y centradas en el usuario."""
        ...
        for art in articles:
            ref = format_reference_apa7(art)
            clean_ref = pdf.clean_text_for_pdf(pdf.safe_text(ref))
            pdf.reference_paragraph(clean_ref)
