...
from app.llm.rag_analyzer import RAGAnalyzer
...
# Bases de datos REALMENTE utilizadas por el sistema (las únicas que debe mencionar)
ACTIVE_SEARCH_DATABASES = {
    'semantic scholar', 'pubmed', 'europe pmc', 'openalex'
}
...
def extract_article_sources(articles, include_type='databases'):
    ...
    def normalize_and_classify(raw_source):
        ...
        for pattern, canonical in SOURCE_CANONICAL_NAMES.items():
            if pattern in raw_lower:
                # Clasificar según tipo: solo bases ACTIVAS se marcan como database
                if pattern in ACTIVE_SEARCH_DATABASES:
                    return canonical, 'database'
                elif pattern in SEARCH_DATABASES:
                    # Bases de datos no activas se ignoran (no se usaron en la búsqueda)
                    continue
    ...
    fields_to_check = ['source', 'database', 'db_source', 'provenance', 'origin', 'url', 'pdf_url', 'doi_url']
...
def _split_saved_criteria(value: object) -> list[str]:
    """Convierte los criterios escritos por el usuario en filas limpias para el reporte."""
    ...
    if len(raw_parts) <= 1:
        raw_parts = re.split(r";\s+(?=[A-ZÁÉÍÓÚÑ¿])", text)

    criteria: list[str] = []
    for part in raw_parts:
        clean = re.sub(r"^\s*(?:CI|CE)?\s*\d+[\).\:-]\s*", "", part.strip(), flags=re.IGNORECASE)
...
def generate_dynamic_qa_criteria(question, articles, metrics=None):
    ...
    pdf_audit = metrics.get("pdf_audit") or {}
    filter_counts = metrics.get("filter_counts") or {}
    inclusion_rows = _split_saved_criteria(metrics.get("inclusion_criteria_text"))
    exclusion_rows = _split_saved_criteria(metrics.get("exclusion_criteria_text"))

    if pdf_audit or filter_counts or inclusion_rows or exclusion_rows:
        qa_rows: list[list[str]] = []
        total_pdf = int(pdf_audit.get("total", len(articles)) or len(articles))
        ready_pdf = int(pdf_audit.get("ready", 0) or 0)
        uploaded_pdf = int(pdf_audit.get("uploaded", 0) or 0)
        if total_pdf:
            qa_rows.append([
                "QA1",
                "Disponibilidad de texto completo para los estudios incluidos.",
                f"{ready_pdf}/{total_pdf} estudios con texto completo; {uploaded_pdf} incorporados por carga manual de PDF."
            ])

        raw_total = int(filter_counts.get("raw_total", metrics.get("total", 0)) or 0)
        final_count = int(filter_counts.get("final_count", metrics.get("final_included", len(articles))) or len(articles))
        concept_discarded = int(filter_counts.get("concept_discarded", 0) or 0)
        metadata_discarded = int(filter_counts.get("metadata_discarded", 0) or 0)
        if raw_total:
            qa_rows.append([
                "QA2",
                "Trazabilidad del flujo PRISMA aplicado por el sistema.",
                f"{raw_total} registros iniciales; {concept_discarded} excluidos por prefiltro conceptual; {metadata_discarded} por metadatos; {final_count} finales tras filtros."
            ])

        if inclusion_rows or exclusion_rows:
            qa_rows.append([
                "QA3",
                "Correspondencia con los criterios declarados por el investigador.",
                f"{len(inclusion_rows)} criterios de inclusion y {len(exclusion_rows)} criterios de exclusion cargados desde la vista de filtros."
            ])

        source_mode = str(metrics.get("article_source_mode") or "").strip().lower()
        if source_mode:
            origin = "CSV curado por el usuario" if source_mode == "csv_only" else "busqueda automatizada y/o CSV"
            qa_rows.append([
                "QA4",
                "Origen de registros usado para la sintesis.",
                f"Modo de fuente: {origin}; {int(metrics.get('csv_import_count', 0) or 0)} registros importados desde CSV."
            ])
    ...
    try:
        ...
        prompt = f"""Actúa como un Auditor Metodológico de academico de alto nivel.
...
def generate_methodology_intro(question, articles):
    ...
    try:
        ...
        prompt = f"""Actúa como un Auditor Metodológico senior.
...
def generate_dynamic_criteria(topic, articles, start_year, end_year, metrics=None):
    ...
    saved_inclusion = _split_saved_criteria(metrics.get("inclusion_criteria_text"))
    saved_exclusion = _split_saved_criteria(metrics.get("exclusion_criteria_text"))
    if saved_inclusion or saved_exclusion:
        if not saved_exclusion:
            saved_exclusion = ["Registros que no cumplen los criterios de inclusion declarados por el investigador."]
        if not saved_inclusion:
            saved_inclusion = ["Registros que cumplen los criterios de inclusion declarados por el investigador."]
        return saved_exclusion[:8], saved_inclusion[:8]
...
def _normalize_parenthetical_aliases(text: str) -> str:
    """Elimina alias entre parentesis que el LLM copia literalmente al titulo."""
    if not text:
        return ""

    clean = str(text)
    clean = re.sub(
        r"aprendizaje\s+autom[aá]tico\s*\(\s*machine\s+learning\s*/\s*deep\s+learning\s*\)",
        "aprendizaje automático y aprendizaje profundo",
        clean,
        flags=re.IGNORECASE,
    )
    clean = re.sub(
        r"machine\s+learning\s*\(\s*machine\s+learning\s*/\s*deep\s+learning\s*\)",
    ...
    def drop_alias(match: re.Match) -> str:
        ...
        if "/" in inner or any(term in inner.lower() for term in alias_terms):
            return ""
        return match.group(0)

    clean = re.sub(r"\s*\(([^)]{3,80})\)", drop_alias, clean)
    clean = re.sub(r"\s+", " ", clean).strip()
    return clean
...
def generate_title_with_blocker(question, articles):
    ...
    tema_principal = _normalize_parenthetical_aliases(extract_main_topic(question))
    ...
    if not tema_principal or len(tema_principal.strip()) < 5:
        ...
        tema_principal = _normalize_parenthetical_aliases(clean_question_for_title(question))
    
    # Construir título en formato estándar RSL
    titulo_es = _normalize_parenthetical_aliases(f"{tema_principal}: Una Revisión Sistemática de la Literatura")
...
def clean_question_for_title(question):
    ...
    return _normalize_parenthetical_aliases(clean.strip())
...
def extract_main_topic(question):
        ...
        if match:
            ...
            if groups:
                ...
                if len(topic) > 5:
                    ...
                    return _normalize_parenthetical_aliases(topic)
    ...
    if len(significant_words) >= 2:
        ...
        return _normalize_parenthetical_aliases(_smart_capitalize_title(result))
    
    # Último fallback: usar parte de la pregunta original
    clean_q = re.sub(r'^[\?]+|[\?]+$', '', clean_q).strip()
    if len(clean_q) > 10:
        return _normalize_parenthetical_aliases(_smart_capitalize_title(clean_q.strip()))
...
def ultra_clean_title(text):
    ...
    text = _normalize_parenthetical_aliases(clean_lines[0])
...
def load_domain_config(domain):
    ...
    if not os.path.exists(config_path):
        return {
            ...
            "term_mappings": {},
            "forbidden_phrases": {
                "nuestro estudio": "el presente estudio",
                "nuestra revisión": "la presente revisión",
            },
        }
...
class ImprovedTranslator:
    ...
    def translate_abstract(cls, text_es):
        ...
        try:
            model = LocalModel.get_instance()
            instruction = """Actúa como un Traductor Académico Senior de nivel academico de alto nivel.
...
TRANSLATE TO ENGLISH:"""
    ...
    def translate_title(cls, text_es):
        ...
        try:
            model = LocalModel.get_instance()
            prompt = f"""Actúa como un Traductor Académico Senior de nivel academico de alto nivel.
...
English Title:"""
        ...
        translated = _normalize_parenthetical_aliases(translated)
        translated = re.sub(
            r"machine\s+learning\s*\(\s*machine\s+learning\s*/\s*deep\s+learning\s*\)",
            "machine learning and deep learning",
            translated,
...
def generate_complete_abstract(question: str, articles: list, stats: dict, metrics: dict) -> str:
    """Generates a structured quantitative abstract using HF API (Boy-Guillén style)."""
    metrics = metrics or {}
    stats = stats or {}
    total_articles = int(metrics.get('final_included', len(articles)) or len(articles))
    filter_counts = metrics.get('filter_counts') or {}
    initial_records = int(
        metrics.get('csv_import_count')
        or filter_counts.get('raw_total')
    ...
    source_method_text = (
        f"Se examinaron {initial_records} registros iniciales importados desde un CSV curado por el usuario y, "
        f"tras la aplicacion de criterios de seleccion, se incluyeron {total_articles} articulos de investigacion."
        if str(metrics.get("article_source_mode") or "").lower() == "csv_only"
        else f"Se examinaron {initial_records} registros iniciales recuperados desde {sources_str} y, "
             f"tras la aplicacion de criterios de seleccion, se incluyeron {total_articles} articulos de investigacion."
    )
    ...
    abstract = (
        f"Esta revision sistematica de la literatura tuvo como objetivo sintetizar y analizar {topic}. "
        f"{source_method_text} {years_text} {journals_text} {models_text} {metrics_text} "
        "En conjunto, la evidencia permite organizar los enfoques, modelos, metricas y condiciones de aplicacion "
        "reportadas por los estudios seleccionados, sin extrapolar hallazgos fuera del corpus incluido."
    )

    return clean_generated_text(replace_anglicisms(abstract))


def _join_spanish_list(parts: list[str]) -> str:
    """Une elementos con coma y conjuncion final."""
    if not parts:
        return ""
    if len(parts) == 1:
        return parts[0]
...
def _build_years_text(years_data: list, total: int) -> str:
    """Construye texto de distribución de años con porcentajes que suman 100%."""
    valid_items = []
    for item in years_data or []:
        year = _coerce_year(item.get('label', '') if isinstance(item, dict) else item)
        ...
        if isinstance(pct, str):
            pct = float(pct.replace('%', '') or 0)
        valid_items.append((year, count, float(pct)))

    if valid_items:
        parts = []
        for year, count, pct in sorted(valid_items, key=lambda data: data[0]):
            count_text = f"{count} articulos" if count != 1 else "1 articulo"
            parts.append(f"{year} ({count_text}; {pct:.1f}%)")
        return f"La distribucion temporal exacta de los estudios incluidos fue: {_join_spanish_list(parts)}."

    return "La metadata de anio de publicacion no estuvo disponible para todos los articulos."


def _coerce_year(value: Any) -> Optional[int]:
    """Convierte un valor de metadata a anio valido, si existe."""
...
def _year_bounds_from_articles_and_stats(articles: list, stats: dict) -> tuple[Optional[int], Optional[int]]:
    """Obtiene el rango temporal real desde articulos finales y stats."""
    years = []
    ...
    for item in (stats or {}).get("years", []) or []:
        if isinstance(item, dict):
            year = _coerce_year(item.get("label"))
        else:
...
def _years_present_from_articles_and_stats(articles: list, stats: dict) -> list[int]:
    """Lista los anios reales presentes en la muestra final."""
    years = set()
    for article in articles or []:
        year = _coerce_year(article.get("year"))
        if year:
...
def _enforce_abstract_temporal_range(text: str, articles: list, stats: dict) -> str:
    """Evita que el refinamiento LLM cambie el periodo real de la muestra."""
    start_year, end_year = _year_bounds_from_articles_and_stats(articles, stats)
    if not text or not start_year or not end_year or start_year == end_year:
        return text

    real_range = f"entre {start_year} y {end_year}"
    real_dash_range = f"{start_year}-{end_year}"
    years_present = _years_present_from_articles_and_stats(articles, stats)
    coverage_text = f"publicaciones distribuidas en el periodo {real_dash_range}"
    if years_present:
        years_text = ", ".join(str(y) for y in years_present[:-1])
        years_text = f"{years_text} y {years_present[-1]}" if years_text else str(years_present[-1])
        coverage_text = f"publicaciones distribuidas en el periodo {real_dash_range}, con presencia de estudios en {years_text}"

    concentration_patterns = [
        r"una\s+concentraci[oó]n\s+significativa\s+de\s+publicaciones\s+en\s+los\s+a[nñ]os\s+(?:19|20)\d{2}(?:\s*,\s*(?:19|20)\d{2})*(?:\s+y\s+(?:19|20)\d{2})?",
        r"publicaciones\s+en\s+los\s+a[nñ]os\s+(?:19|20)\d{2}(?:\s*,\s*(?:19|20)\d{2})*(?:\s+y\s+(?:19|20)\d{2})?",
    ]
    fixed = text
    for pattern in concentration_patterns:
        fixed = re.sub(pattern, coverage_text, fixed, flags=re.IGNORECASE)
    range_pattern = re.compile(r"entre\s+(?:19|20)\d{2}\s+(?:y|-|a)\s+(?:19|20)\d{2}", re.IGNORECASE)

    def replace_if_temporal_context(match: re.Match) -> str:
        window = fixed[max(0, match.start() - 120):match.start()].lower()
        temporal_terms = (
            "articul", "estudi", "public", "revision", "literatura",
            "seleccion", "incluid", "period", "temporal", "registro"
        )
        if any(term in window for term in temporal_terms):
            return real_range
        return match.group(0)

    fixed = range_pattern.sub(replace_if_temporal_context, fixed)
    if real_range not in fixed.lower() and real_dash_range not in fixed:
        fixed = re.sub(
            r"(art[ií]culos de investigaci[oó]n(?:\s+que)?\s+cumplieron[^.]*\.)",
            rf"\1 El periodo cubierto por los estudios seleccionados fue {real_dash_range}.",
            fixed,
            count=1,
            flags=re.IGNORECASE,
        )
...
def _enforce_abstract_source_language(text: str, metrics: dict) -> str:
    """Distingue origen de registros y medio de publicacion en modo CSV."""
    if str((metrics or {}).get("article_source_mode") or "").lower() != "csv_only":
        return text
    text = re.sub(
        r"\bfuentes identificadas\b",
        "revistas o medios de publicacion identificados",
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(
        r"\blas fuentes\b",
        "las revistas o medios de publicacion",
        text,
        flags=re.IGNORECASE,
    )
    return text
...
def _build_journals_text(journals_data, total):
    """Construye texto de distribucion editorial sin confundir revistas con bases de datos."""
    if not journals_data:
        return "La metadata editorial corresponde a revistas o medios de publicacion especializados."
    ...
    if remaining > 0.5:
        parts.append(f"mientras que el {remaining:.1f}% restante se distribuyo en otras revistas")

    if len(parts) == 1:
        return f"En relacion con las revistas o medios de publicacion, se registro {parts[0]}."
    if len(parts) == 2:
        return f"En relacion con las revistas o medios de publicacion, se registro {parts[0]} y {parts[1]}."
    return f"En relacion con las revistas o medios de publicacion, se registro {parts[0]}, {parts[1]}, {parts[2]}."
...
def refine_introduction(text, protected_terms=None):
    ...
    # Ejemplo generico: "Autor et al. (2022) ... (Autor et al., 2022)." -> cita narrativa unica.
    ...
    def _run_refinement_pass(input_text, issues_list, pass_number):
        ...
        try:
            ...
            refine_prompt = f"""Eres un corrector de estilo academico senior para revistas specializado. Pase de correccion #{pass_number}.
...
class AcademicRefiner:
    ...
    def refine(self, text, context="intro", mirror_mode=False):
        ...
        if context == "abstract":
            if is_english:
                instruction = """Act as a Senior Academic Copyeditor for academico de alto nivel journals.
...
Only respond with the CORRECTED TEXT in a single paragraph."""
                ...
                instruction = """Actúa como un Corrector de Estilo Académico para revistas academico de alto nivel.
...
Solo responde con el TEXTO CORREGIDO en un solo párrafo."""
             ...
             instruction = """Actúa como un Corrector de Estilo Académico Senior para revistas academico de alto nivel.
...
1. NUNCA fijes aportes, tecnicas ni autores de un dominio ajeno al corpus entregado.
2. Asocia cada hallazgo unicamente al autor y año que aparecen en la lista de articulos.
3. Si el abstract no permite extraer un aporte claro, escribe "aporte no determinado".
...
def build_funnel_introduction(question, articles, stats, specific_objectives=None):
    ...
    if not real_sources:
        fallback_sources = get_sources_summary(articles).get('all') or []
        real_sources = fallback_sources or ["fuentes academicas registradas en la sesion"]
        logging.warning("No se detectaron bases de busqueda explicitas; se usara fallback de fuentes.")
    ...
    instruction = f"""Actua como un Investigador Academico Senior (PhD) escribiendo para una revista academico de alto nivel.
...
- El lector academico de alto nivel ya reconoce el tema; la redundancia es una falta de profesionalismo.
...
"""
    ...
    try:
        ...
        # 7. CAPA DE REFINAMIENTO ACADÉMICO FINAL (academico de alto nivel)
...
def _clean_rsl_topic_label(topic: str) -> str:
    """Convierte la pregunta del usuario en una frase de dominio reusable sin fijar un tema."""
    clean = _normalize_parenthetical_aliases(replace_anglicisms(str(topic or ""))).strip()
    clean = re.sub(r"^[¿?\s]*(cu[aá]les?|qu[eé]|c[oó]mo|de qu[eé] manera)\s+", "", clean, flags=re.IGNORECASE)
    clean = re.sub(r"^(son|es|se|pueden|permite|permiten|influye|influyen)\s+", "", clean, flags=re.IGNORECASE)
    clean = re.sub(r"\s+", " ", clean).strip(" .:;¿?")
    if not clean:
        return "el tema de estudio"
    return clean[0].lower() + clean[1:]


def _domain_agnostic_research_questions(topic: str, articles: Optional[list] = None) -> list[list[str]]:
    """PI deterministas para RSL: mismas dimensiones metodologicas, dominio tomado del usuario."""
    scope = _clean_rsl_topic_label(topic)
    return [
        [
            "1",
            "Datos y evidencia",
            f"PI1: ¿Qué tipos de datos, evidencia o fuentes utiliza la literatura sobre {scope}?",
            f"Identificar los tipos de datos, evidencia y fuentes empleados en los estudios sobre {scope}.",
        ],
        [
            "2",
            "Modelos y técnicas",
            f"PI2: ¿Qué modelos, técnicas o enfoques metodológicos se reportan para {scope}?",
            f"Caracterizar los modelos, técnicas y enfoques metodológicos reportados para {scope}.",
        ],
        [
            "3",
            "Estrategias de mejora",
            f"PI3: ¿Qué estrategias de optimización, adaptación o mejora se aplican en los estudios sobre {scope}?",
            f"Analizar las estrategias de optimización, adaptación o mejora aplicadas en la literatura sobre {scope}.",
        ],
        [
            "4",
            "Evaluación del rendimiento",
            f"PI4: ¿Qué métricas, criterios o indicadores se utilizan para evaluar los resultados relacionados con {scope}?",
            f"Determinar las métricas, criterios e indicadores utilizados para evaluar los resultados relacionados con {scope}.",
        ],
        [
            "5",
            "Aportes y limitaciones",
            f"PI5: ¿Qué aportes, beneficios y limitaciones identifica la literatura sobre {scope}?",
            f"Sintetizar los aportes, beneficios y limitaciones identificados en la literatura sobre {scope}.",
        ],
    ]


def get_specific_research_questions(topic: str, articles: Optional[list] = None) -> list[list[str]]:
    ...
    """
    topic = _normalize_parenthetical_aliases(topic)
    return _domain_agnostic_research_questions(topic, articles)
...
1. Equilibrio de Dominio: Los temas deben abordar el fenómeno desde una perspectiva integral (datos/evidencia, mecanismos/técnicas, evaluación, comparación e impacto/beneficios), evitando tecnicismos extremos de una sola carrera a menos que se solicite.
...
5. Redacción: Preguntas breves, directas y sin ejemplos entre paréntesis. No copies alias del título como "(Machine Learning / Deep Learning)"; usa términos limpios en español.
6. Temporalidad: No generes una PI de evolución temporal ni frases como "en los últimos 5 años" salvo que el título lo pida explícitamente.
        ...
        if '[' in clean:
            ...
            if isinstance(rows, list):
                ...
                for i, r in enumerate(rows[:5]):
                    ...
                    tema = _normalize_parenthetical_aliases(replace_anglicisms(str(r[1])))
                    if len(tema.split()) > 5:
                        tema = " ".join(tema.split()[:4])
                    
                    # 2. PREGUNTA: Formato PI_n: ¿...?
                    pregunta_raw = _normalize_parenthetical_aliases(replace_anglicisms(str(r[2]))).strip()
                    pregunta_raw = re.sub(r"\s*\((?:ej\.?|e\.g\.|incluyendo|incl\.?)[^)]+\)", "", pregunta_raw, flags=re.IGNORECASE)
                    if not re.search(r"(evoluci[oó]n|tendencia\s+temporal|[uú]ltimos\s+\d+)", topic, re.IGNORECASE):
                        pregunta_raw = re.sub(r"\s+en\s+los\s+[uú]ltimos\s+\d+\s+a[nñ]os", "", pregunta_raw, flags=re.IGNORECASE)
                    ...
                    objetivo = _normalize_parenthetical_aliases(replace_anglicisms(str(r[3])))
                    objetivo = re.sub(r"\s*\((?:ej\.?|e\.g\.|incluyendo|incl\.?)[^)]+\)", "", objetivo, flags=re.IGNORECASE)
                    if not re.search(r"(evoluci[oó]n|tendencia\s+temporal|[uú]ltimos\s+\d+)", topic, re.IGNORECASE):
                        objetivo = re.sub(r"\s+en\s+los\s+[uú]ltimos\s+\d+\s+a[nñ]os", "", objetivo, flags=re.IGNORECASE)
...
def generate_synthesis_full(articles: list, question: str, metrics: dict) -> dict:
    ...
    try:
        ...
        refiner = AcademicRefiner()
        # El resumen ya es deterministico; no se reescribe con LLM para evitar cambios de anios o conteos.
        resumen_es = _enforce_abstract_temporal_range(resumen_es, articles, stats)
        resumen_es = _enforce_abstract_source_language(resumen_es, metrics)
...
def generate_evidence_based_introduction(question, articles, stats, specific_objectives=None):
    ...
    try:
        ...
        logging.warning(f"Introduccion funnel no disponible; se usara fallback breve: {e}")
...
def _format_stat_items(items: list, limit: int = 4) -> str:
    """Formatea una lista de estadisticas ya calculadas."""
    parts = []
    for item in (items or [])[:limit]:
        label = str(item.get('label', '')).strip() if isinstance(item, dict) else str(item)
        pct = item.get('percentage', 0) if isinstance(item, dict) else 0
        if isinstance(pct, str):
            pct = float(pct.replace('%', '') or 0)
        if label:
            parts.append(f"{label} ({pct:.1f}%)")
    return _join_spanish_list(parts)


def generate_general_description_rag(articles: list, question: str, stats: dict) -> str:
    """
    ...
    """
    total = len(articles)
    years = [str(item['label']) for item in stats.get('years', []) if item.get('label') != 'N/D']
    year_range = f"{min(years)}-{max(years)}" if years else "sin rango temporal declarado"
    topic = extract_main_topic(question)
    methods_text = _format_stat_items(stats.get('methodology_types', []), 4)
    themes_text = _format_stat_items(stats.get('themes', []), 3)

    description = (
        f"La presente revision sistematica analiza {total} estudios seleccionados sobre {topic}, "
        f"publicados en el periodo {year_range}. "
        f"{_build_years_text(stats.get('years', []), total)} "
    )
    if methods_text:
        description += f"La clasificacion operativa de titulos y resumenes identifica {methods_text}. "
    if themes_text:
        description += f"Los temas tecnicos mas frecuentes fueron {themes_text}. "
    description += (
        "Estos resultados se reportan como sintesis descriptiva del corpus incluido y no incorporan inferencias "
        "externas a los metadatos, titulos y resumenes disponibles."
    )
    return description
...
- Distribucion operativa detectada en titulos/abstracts: {', '.join([f"{m['label']} ({m['percentage']}%)" for m in stats.get('methodology_types', [])])}
...
def extract_years_info_corrected(articles: list) -> str:
    ...
    if years:
        ...
        for year in sorted(counter):
            count = counter[year]
            pct = (count / len(years)) * 100 if years else 0
            count_text = f"{count} articulos" if count != 1 else "1 articulo"
            parts.append(f"{year} ({count_text}; {pct:.1f}%)")
        return f"Los estudios abarcan el periodo {min(years)}-{max(years)}; la distribucion exacta es: {_join_spanish_list(parts)}."
...
def generate_evidence_based_discussion(articles, question, stats):
    ...
    temporal_text = (
        _build_years_text(years_data, total_articles).rstrip(".")
        if years_data
        else "La metadata temporal no permite afirmar una concentracion por anio"
    )
    p1 = (
        f"Los hallazgos de esta revision sistematica sobre {topic} revelan tendencias en el corpus analizado. "
        f"Del analisis de {total_articles} articulos seleccionados, {tech_text}. {temporal_text}."
    )
    ...
    p2 = f"""La clasificacion operativa detectada en titulos y resumenes incluye: {methods_text}. Esta lectura debe interpretarse como una ayuda descriptiva del corpus, no como una evaluacion metodologica manual de cada articulo."""
