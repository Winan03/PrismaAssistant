...
def call_ai_api(instruction: str, user_prompt: str = "", max_tokens: int = 250, system_prompt: str = None) -> Dict:
    ...
    logging.info(f"🔍 [DEBUG call_ai_api] Raw content ({len(content)} chars): {repr(content[:1000])}")
...
def get_column_aware_context(full_text: str, abstract: str, col_label: str, col_instruction: str, research_question: str = "", chunk_offset: int = 0) -> str:
    ...
    from app.screening.screening import get_embedding, cosine_similarity
...
def translate_question_to_english(text: str) -> str:
    """Extrae palabras clave en inglés de la pregunta de investigación para el Cross-Encoder"""
    prompt = f"Act as a search engineer. Extract the 5 to 8 most important technical keywords from the following research question and translate them to English. DO NOT write a full sentence. DO NOT use question marks. ONLY output a space-separated string of keywords.\n\n{text}"
    max_retries = 3
    for attempt in range(max_retries):
        try:
            response = client.models.generate_content(
            ...
            logging.warning(f"⚠️ Error extrayendo keywords en inglés (intento {attempt+1}): {e}")
            if attempt < max_retries - 1:
                wait_time = 2 ** attempt
                logging.info(f"🔄 Reintentando en {wait_time}s...")
                time.sleep(wait_time)
                
    logging.error("❌ Fallaron todos los intentos. Usando texto original.")
    return text
