...
class Provider(Enum):
    GEMINI = "gemini"
    GEMMA_27B = "gemma_27b"             # v16.3: Gemma 3 27B — 30 RPM, 14.4K RPD gratis
    GEMMA_12B = "gemma_12b"             # v16.3: Gemma 3 12B — 30 RPM, 14.4K RPD gratis
    CEREBRAS = "cerebras"               # v11.12: Llama 3.1 8B — 1M tokens/día
    GROQ = "groq"                       # v11.13: Llama 3.3 70B
    GITHUB_GPT4O = "github_gpt4o"       # v15.7: GPT-4o via GitHub Models
    GITHUB = "github"                   # v11.20: GitHub Models (fallback)
    OPENROUTER = "openrouter"           # v11.23: Modelos 100% gratuitos
        ...
        with self._api_semaphore:
            for provider in PROVIDER_ORDER:
                ...
                for i, key in enumerate(keys):
                    logging.info(f"\U0001f504 Intentando con {cfg['name']} (Key {i+1}/{len(keys)})...")
            ...
            if response.status_code == 200:
                ...
                if 'choices' in result and len(result['choices']) > 0:
                    ...
                    if finish_reason in ('length', 'max_tokens'):
                        logging.warning(f"⚠️ [Gemini] finish_reason={finish_reason} con {len(text)} chars — max_tokens demasiado bajo?")
                    if text:
